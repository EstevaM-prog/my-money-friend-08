...
// The BudgetRuleFunc type is an adapter to allow the use of ordinary
// function as BudgetRule mutator.
type BudgetRuleFunc func(context.Context, *ent.BudgetRuleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BudgetRuleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.BudgetRuleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BudgetRuleMutation", m)
}
