...
type UserQuery struct {
	...
	withBudgetRules  *BudgetRuleQuery
...
// QueryBudgetRules chains the current query on the "budget_rules" edge.
func (_q *UserQuery) QueryBudgetRules() *BudgetRuleQuery {
	query := (&BudgetRuleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBudgetRules:  _q.withBudgetRules.Clone(),
...
// WithBudgetRules tells the query-builder to eager-load the nodes that are connected to
// the "budget_rules" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBudgetRules(opts ...func(*BudgetRuleQuery)) *UserQuery {
	query := (&BudgetRuleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBudgetRules = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBudgetRules; query != nil {
		if err := _q.loadBudgetRules(ctx, query, nodes,
			func(n *User) { n.Edges.BudgetRules = []*BudgetRule{} },
			func(n *User, e *BudgetRule) { n.Edges.BudgetRules = append(n.Edges.BudgetRules, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadBudgetRules(ctx context.Context, query *BudgetRuleQuery, nodes []*User, init func(*User), assign func(*User, *BudgetRule)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.BudgetRule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BudgetRulesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_budget_rules
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_budget_rules" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_budget_rules" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
