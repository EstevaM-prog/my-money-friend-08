...
// HasBudgetRules applies the HasEdge predicate on the "budget_rules" edge.
func HasBudgetRules() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasBudgetRulesWith applies the HasEdge predicate on the "budget_rules" edge with a given conditions (other predicates).
func HasBudgetRulesWith(preds ...predicate.BudgetRule) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newBudgetRulesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
