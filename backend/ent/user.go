...
type UserEdges struct {
	...
	// BudgetRules holds the value of the budget_rules edge.
	BudgetRules []*BudgetRule `json:"budget_rules,omitempty"`
...
// BudgetRulesOrErr returns the BudgetRules value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) BudgetRulesOrErr() ([]*BudgetRule, error) {
	if e.loadedTypes[4] {
		return e.BudgetRules, nil
	}
	return nil, &NotLoadedError{edge: "budget_rules"}
}
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldName, user.FieldEmail, user.FieldPassword, user.FieldPhone, user.FieldAvatarURL, user.FieldRole:
...
// QueryBudgetRules queries the "budget_rules" edge of the User entity.
func (_m *User) QueryBudgetRules() *BudgetRuleQuery {
	return NewUserClient(_m.config).QueryBudgetRules(_m)
}
