...
import { INITIAL_TRANSACTIONS, CATEGORY_COLORS, Transaction } from "@/lib/finance-data";
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-card rounded-xl p-5 card-shadow">
          ...
          <p className="text-xl font-bold mt-1 text-primary">{fmt(totalIncome)}</p>
        </div>
        <div className="bg-card rounded-xl p-5 card-shadow">
          <p className="text-sm text-muted-foreground">Total Despesas</p>
          <p className="text-xl font-bold mt-1 text-destructive">{fmt(totalExpense)}</p>
        </div>
        <div className="bg-card rounded-xl p-5 card-shadow">
          <p className="text-sm text-muted-foreground">Saldo Período</p>
          <p className="text-xl font-bold mt-1 text-balance">{fmt(totalIncome - totalExpense)}</p>
        </div>
        <div className="bg-card rounded-xl p-5 card-shadow">
      ...
      {/* Table breakdown */}
      <div className="bg-card rounded-xl p-5 card-shadow">
        <h3 className="text-base font-semibold mb-4">Detalhamento por Categoria</h3>
        <Table>
          <TableHeader>
            <TableRow>
                ...
                <TableRow key={cat.name}>
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      <div className="h-3 w-3 rounded-full" style={{ backgroundColor: CATEGORY_COLORS[cat.name] }} />
                      {cat.name}
                    </div>
                  </TableCell>
                  <TableCell className="text-right">{fmt(cat.value)}</TableCell>
                  <TableCell className="text-right">{totalExpense > 0 ? ((cat.value / totalExpense) * 100).toFixed(1) : 0}%</TableCell>
