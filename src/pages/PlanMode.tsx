  ...
  return (
    <div className="min-h-screen bg-[#020205] text-white flex flex-col items-center justify-center py-20 px-6 font-sans overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto w-full space-y-20 relative z-10 font-sans">
        ...
        {/* Pricing Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch lg:px-10">
          {plans.map((plan, index) => (
            <div 
              key={plan.name}
              className={cn(
                "group relative rounded-[2rem] flex flex-col transition-all duration-700 overflow-hidden border border-white/[0.06] bg-[#090910]/40 backdrop-blur-3xl",
                plan.highlight 
                  ? "ring-1 ring-purple-500/20 shadow-[0_0_80px_-20px_rgba(139,92,246,0.15)] md:-translate-y-4" 
                  : "hover:border-white/10"
              )}
            >
              {/* Header area with potential gradient */}
              <div className={cn(
                "p-8 lg:p-10 space-y-6 relative",
                plan.highlight && "bg-gradient-to-br from-purple-900/20 to-transparent"
              )}>
                {plan.highlight && (
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-white/50 tracking-wide uppercase">{plan.name}</p>
                    {plan.badge && (
                      <span className="inline-block px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-[10px] font-bold text-purple-400 uppercase tracking-widest">
                        {plan.badge}
                      </span>
                    )}
                  </div>
                ...
                <div className="space-y-1">
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-sm text-white/30 font-semibold">{plan.period}</span>
                  </div>
                ...
                <Button 
                  asChild
                  className={cn(
                    "w-full h-14 rounded-2xl text-[15px] font-bold transition-all duration-500 border border-white/10",
                    plan.highlight 
              ...
              {/* Features List */}
              <div className="p-8 lg:p-10 flex-1 space-y-6">
                ...
                <ul className="space-y-5">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-center gap-3.5 group/feature">
                      <div className="shrink-0 w-6 h-6 rounded-full border border-white/10 flex items-center justify-center group-hover/feature:border-white/20 transition-colors">
                        <CheckCircle2 className="h-3.5 w-3.5 text-white/30 group-hover/feature:text-white/60" />
                      </div>
                      <span className="text-[13px] text-white/50 font-medium tracking-tight group-hover/feature:text-white/80 transition-colors">
                        {feature}
                      </span>
                    </li>
                  ))}
